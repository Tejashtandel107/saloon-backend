...
class BookingService {
    ...
    async getAvailableSlots(date: string) {
        ...
        if (day === "Saturday") endHour = 16;
        if (day === "Sunday") throw new AppError("No slots available on Sunday", 400);
...
console.log(data);
    ...
    async cancelBooking(id: string) {
        ...
        if (booking.status === "cancel") throw new AppError("Booking already cancelled", 400);

        if (booking.status === "done") throw new AppError("Booking already completed", 400);
        ...
        emailQueue.add(() =>
            this.sendEmail(booking.email,"Booking Cancelled","bookingCancelled",
                { name: booking.name, email: booking.email, slot }
            )
        );
    ...
    async completeBooking(id: string) {
        ...
        if (booking.status === "cancel") throw new AppError("Cannot complete cancelled booking", 400);
        ...
        emailQueue.add(() =>
            this.sendEmail(booking.email,"Booking Completed","bookingCompleted",
                { name: booking.name, email: booking.email, slot }
            )
        );
